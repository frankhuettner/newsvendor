...
# ╔═╡ 95d5fdd7-9522-4621-b683-c3ab26c8ed82
using PlutoUI,Plots,StatsBase,Distributions

# ╔═╡ 4af89c00-1f59-11ec-139b-6d24cfe92543
story = """
...
# ╔═╡ a397b8d6-1bca-4cbb-aec4-8fbf528bf645
story; Markdown.parse_file("cheers_1_story.md")

# ╔═╡ 010e4367-6866-4a08-8014-bc3ef263577d
begin
	# Output story in a md file
	story
	open("cheers_1_story.md", "w") do file
		write(file, story)
	end
end

# ╔═╡ 5a81054c-ed58-4bd3-83dc-887247757778
begin
...
#demands = rand(Normal(90,50), 360) 		

end

# ╔═╡ ba704983-8a8a-477d-8b65-ca32aa06e5a3
begin
	observed_demands = Int.( replace(x -> x>100 ? 100 : round(Int, x), demands) )
	observed_demands = Int.( replace(x -> x<0 ? 0 : round(Int, x), observed_demands) )
end

# ╔═╡ 6972d82d-dba2-459e-af7c-7799bf428dbc
begin 
	
	plt = histogram(observed_demands, bins=5:10:175, dpi=400, size=(700,500),
			label="Observed frequency",  xlabel="Cakes Sold",
			bottom_margin=0Plots.mm, top_margin=0Plots.mm, right_margin=0Plots.mm, 
			left_margin=6Plots.mm, yticks=0:10:175,
			xtickfontsize=12, ytickfontsize=12, legendfontsize=10, labelfontsize = 16)
	savefig(plt,"img/cheers_1_story_histogram.svg"	)
	
	heigth_at_100 = fit(Histogram, observed_demands, 5:10:175).weights[10]
	
	
	col_hist_at_100 = 360 * (1 - cdf(Normal(90,50), 100))
	col_hist_at_100_without_cutoff = 360 * (cdf(Normal(90,50), 105) - 
												cdf(Normal(90,50), 95))
		
	
	col_hist_at_10to170_without_rnd = 360 * (cdf(Normal(90,50), 15:10:175) .- 
											cdf(Normal(90,50), 5:10:165))
	plot!(10:10:170, col_hist_at_10to170_without_rnd, lw=5, label="Normal curve\n(μ=90, σ=30)", c=4)
	
	col_hist_at_100to170_without_cutoff = 360 * (cdf(Normal(90,50), 105:10:175) .- 
											cdf(Normal(90,50), 95:10:165))
	sum_100to170_without_cutoff = sum(col_hist_at_100to170_without_cutoff)
	
	col_hist_at_100to170_without_cutoff = heigth_at_100/sum_100to170_without_cutoff .* col_hist_at_100to170_without_cutoff
	
	bar!(100:10:170,  col_hist_at_100to170_without_cutoff, alpha=.34, label="Splitting the\ndemand at 100\nas if there was\nno stock-out", c=1, lw=3)
	
	
	savefig(plt,"img/cheers_1_story_histogram_with_curve.svg")
	plt	
end

# ╔═╡ a3b33298-f402-4f33-a4a3-0056e01ded4d
fit(Histogram, observed_demands, 5:10:175).weights[10]

# ╔═╡ 7bd0cbfc-c659-4cf1-a2bf-d584debe76e1
demands

# ╔═╡ a6009bb6-705a-4971-a71c-c8845d9eba0b
html"""
	<style>
