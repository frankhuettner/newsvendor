...
# ╔═╡ 5ab57126-5f32-4439-b560-3e734a6d1972
using PlutoUI, Distributions, QuadGK, OrderedCollections, JSON3, Tables, Configurations,Plots; import TOML

# ╔═╡ 56a87a4e-2005-11ec-0889-abed41b6a9c3
md" Installation of packages might take about 10 minutes.
Installation is compeleted if this animations stops

↙️
"

# ╔═╡ 4510b0fc-2864-4f88-827b-234a25399d6a
md" Meanwhile, use your time and read the case and the explanations below.
"
...
![](https://cdn.pixabay.com/photo/2019/10/08/21/08/cake-4536039_1280.jpg)
...
``\qquad \qquad \qquad``**👉 How many cakes you do you want to make for this day?**

Note that you can only make this decision once per day. **Baking further cakes during the day is not possible**.
...
begin		
	scenario = Scenario(u=180, σ=30, c=1, p=5, 
							delay=30, max_num_days=30, 
							allow_reset=false,
							title="Patisserie Cheers! (Part I)", 
	...
	function show_scenario_data(scenario::Scenario)
		...
		u = scenario.u
		l = scenario.l
		distr = scenario.distr
		max_num_days = scenario.max_num_days
		text = md"""
		...
		* Independent of the demand of the previous day, you **expect μ = $(my_round(μ))** and face uncertainty captured by a standard deviation of **σ = $(my_round(σ))**. The distribution is shown in the figure to the right).
		* At the end of each round, unsold units are discarded at a **salvage value** of **\$$(s)**.
		* The simulation lasts for **$max_num_days rounds**.
		"""
		...
		plot!(plt_dem, l:u, pdf(distr,l:u), fillrange=(x->0), fillalpha=0.1,  lw=3)
		plot!(plt_dem, l-(u-l)/5:l, x->0, lw=3, c=1)
		plot!(plt_dem, u:u+(u-l)/5, x->0, lw=3, c=1)
		vline!([μ])
		annotate!(0.98*μ, pdf(distr,μ)/5, ("μ = $(my_round(μ))", 8, :purple, :right))
		
	
	
		plt_econ = bar([1], [s], 
			orientation=:h, yaxis=false,yticks=nothing, c = :grey, alpha=.3, legend=false)	
		bar!([2], [c], orientation=:h, c = :red, alpha=.3, legend=false)
		bar!([3], [p], orientation=:h, c = :green, alpha=.3, legend=false)
		annotate!(0.02*p, 3, "Unit Selling Price", :left)
		annotate!(0.02*p, 2, "Unit Cost", :left)
		annotate!(0.02*p, 1, "Unit Salvage Value", :left)
		
		plt = plot(plt_dem, plt_econ, layout=(2,1), dpi=300, size=(360,360))
		# savefig(plt,"img/handout_data_viz.png")
		return TwoColumn(text, plt)
	end; 
	
	show_scenario_data(scenario)
end
...
$(@bind q1a CheckBox())Cakes are prepared in the morning and in the afternoon (if demand is high enough).

$(@bind q1b CheckBox())Cakes are prepared in the morning only.

$(@bind q1c CheckBox())Left-over cakes are resold on the next day at a discounted
price for € 0.5.

$(@bind q1d CheckBox())Left-over cakes are donated at a salvage value of zero.

"""

# ╔═╡ 40b63359-e350-4fe3-b7f2-ffcf66364b2a
TwoColumn( md""" ### Question 2. 
	
``\qquad \qquad``$(@bind q2 MultiSelect(["a", "b"]))"""
	,
md"""
	
Given your demand information, you are 95% confident that daily demand will be roughly between...



a) 90 +/- 2*30, i.e., between 30 and 150

b) 90 +/- 30, i.e., between 60 and 120


"""
)

# ╔═╡ 607ebae3-89f3-4384-b7e3-e9f282e7b2c8
md"*In case you struggel with question 2, please watch [the video below](https://youtu.be/mtbJbDwqWLE) explaining the 68-95-97.5 rule:*"

# ╔═╡ b1459892-4d8b-45d1-9405-c814695935ea
html"""

<div style="padding:56.25% 0 0 0;position:relative;"><iframe src="https://www.youtube.com/embed/mtbJbDwqWLE" style="position:absolute;top:0;left:0;width:100%;height:100%;" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe></div>
"""

# ╔═╡ 7f1d8f16-8d0d-4b18-881a-e812998b5548
TwoColumn( md""" ### Question 3. 
	
``\qquad \qquad``$(@bind q3 MultiSelect(["a", "b"]))"""
	,
md"""
	
Given your demand information, daily demand will be lower than 120 cakes with probability...
...
# ╔═╡ f642deeb-9866-45f5-baa1-1839ca4fd88e
md""" ### Question 4.
	

A cake sells for € $(@bind q4a NumberField(0.00 : 0.1 : 10.0)) and is produces with variable cost of € $(@bind q4b NumberField(0.00 : 0.1 : 10.0)).
"""

# ╔═╡ 599d2cc5-a39f-4c05-aced-c54ba21c4513
TwoColumn( md""" ###  Question 5. 
	
``\qquad \qquad``$(@bind q5 MultiSelect(["a", "b"]))"""
	,
md"""
	
If you make 100 cakes on a specific day and customers demand 40 cakes, then you have 60 cakes leftover and your contribution to profit on that day is... 
...
yays = [md"Great!", md"Yay ❤", md"Great! 🎉", md"Well done!",  md"Good job!", md"Awesome!", md"You got the right answer!", ]; md""

# ╔═╡ ebc5119f-91c5-45fb-a1f6-bb07038d421b
if !q1a && q1b && !q1c && q1d	rand(yays) end

# ╔═╡ 943d5a8c-a995-4c36-aa13-2c08eced1963
if q2 == ["a"] 	rand(yays) end

# ╔═╡ 377af798-9287-4fef-a4da-8d7027a7cf47
if q3 == ["a"] 	rand(yays) end

# ╔═╡ 95bfef2e-c354-424f-bae7-0003dfb2876b
if q4a == 5.0 && q4b == 1.0 	rand(yays) end

# ╔═╡ 93d6c48e-8897-49f3-8fc1-899d19adff81
if q5 == ["a"] 	rand(yays) end
...
# ╔═╡ 7d5345b6-60c9-4408-8c04-a3e8a8582200
if !show_code
	html"""
	<style>
	pluto-input {
