...
# ╔═╡ 72bb56c9-df2e-4ba6-9231-e4494e2a5a1b
course_path = homedir() * "/data/newsvendor_simulation/" * course_name

# ╔═╡ bf65434a-82a2-4833-8ea8-1fc64c8bb71e
simlog_path = course_path * "/simlog"

# ╔═╡ 91035449-f46c-4b6d-8495-e908d3275b90
nb_path = course_path * "/notebooks"

# ╔═╡ 4230667d-883f-4125-a167-44df80e8f826
md"# 📈📉📈 Individual play"

# ╔═╡ b4e41432-8ab9-4c85-b721-d0959505d51d
md"# 📊 Student's aggregated play"

# ╔═╡ 475f9182-8b8e-414d-aa3c-b0ebb916265c
@bind reload_stud_data CounterButton("Reload data from students")

# ╔═╡ a65151c9-473d-4add-a4cd-6d0aabdc06ab
available_scenarios = ["cheers_1", "cheers_2", "cheers_3", "cheers_4", ]; md"""

Select a scenarios for aggregated performance 👉 $(@bind selected_scenario_agg Select(available_scenarios))
"""

# ╔═╡ 8d2382de-0112-49c0-8fdd-f9c57267f7e9
begin if isfile(course_path * "/df_links.jld2")
	df_res_ind = load(course_path * "/df_links.jld2", "new_df_links")
	insertcols!(df_res_ind, :DaysPlayed => 0, 
						:ExpProfit => 0.0,
						:AvgStock => 0.0, 
						:AvgDemand => 0.0, 
						:TotalProfit => 0.0, 
						)
	students = [  row.token => row.name for row in eachrow(df_res_ind)  ]
	md"""
	Select a scenarios 👉 $(@bind selected_scenario_ind Select(available_scenarios))
	Select a student 👉 $(@bind selected_token Select(students))
...
# ╔═╡ e0aa7009-9b5b-4f9b-b125-22ab17df3f75
if !isfile(course_path * "/df_links.jld2") 
	md"""
	!!! warning "Wait, there are no simulations in this data folder!"
	"""
else
...
# ╔═╡ 5e54a225-7282-4cad-a23a-46fa1f7c251f
...
# ╔═╡ 841604b5-612c-4da6-a331-8a84c2cd7501
...
# ╔═╡ 7ac0dcce-68bc-40eb-98be-0269f6094c19
if @isdefined df_res_ind
	logfile_toml_ind = simlog_path * "/simlog_" * string(selected_token) * ".toml"
	if isfile(logfile_toml_ind)
		...
		md"Expected profit = $(sim_data_ind.expected_profits)"
	end
end

# ╔═╡ e32b2644-cc43-456e-be8a-a9241122870d
if @isdefined sim_data_ind 
	update_plot_panel_1(sim_data_ind)
end

# ╔═╡ b9a29316-50e9-43cc-9df5-c57c6f387a08
if @isdefined sim_data_ind
	update_plot_panel_2(sim_data_ind)
end

# ╔═╡ ca65fc95-2851-412a-9251-62f31a8c5d3f
reload_stud_data; if isfile(course_path * "/df_links.jld2")
	...
	for row in eachrow(df_res)
		...
		logfile_toml_name = "simlog_" * string(row.token) * ".toml"
		if isfile(simlog_path * "/" * logfile_toml_name)
			dict_simlogs = TOML.parsefile(simlog_path * "/" * logfile_toml_name)
	...
	# df_res = df_res[df_res[!, :token].!="4sGdqu1U",:]  # filter specific player
	
	jldsave(course_path * "/df_res.jld2"; df_res)
	
	# df_res[!, filter(x->x !="link" , names(df_res))]
	df_res[!, filter(x->x !="link" , names(df_res))] |> data_table
end

# ╔═╡ efef5281-5106-4802-9191-4a8421fd8371
if @isdefined df_res 
	if nrow(df_res) > 0
		plotly()
		xmax = 1.1* max(maximum(df_res[!,:AvgDemand]),maximum(df_res[!,:AvgStock]))
		# xmax = 200
		plot(title="Each point represents one student", 
			xlabel="Average Demand", ylabel="Average Stock",legend=false,
			aspect_ratio=:equal, xlims=(0,xmax), ylims=(0,xmax),
			size = (700,700)
		)
		scatter!(df_res[!,:AvgDemand], df_res[!,:AvgStock], markersize = 8)
		plot!(1:xmax, ls = :dash, c = :black, lw=3)
	end
end
...
# ╔═╡ c34a36a7-54b7-4adf-a23e-a857bb3a4dee
let
	if @isdefined df_res 
	if nrow(df_res) > 0
		df_profit = copy(df_res)
		sort!(df_profit, [order(:TotalProfit, rev=true)])



		ticklabel = string.(df_profit[!,:name])
		plt_profit = bar(df_profit[!,:TotalProfit],  size=(750,800), dpi=300,
							yticks=(1:length(ticklabel), ticklabel),
			orientation=:h, legend=false, title="Total realized profit", yflip=true)

		vline!([profit_opt*30], lw=1)
...
# ╔═╡ 5f04678b-7a35-460d-bc0b-13c634892320
md"##  Precompile plotting"

# ╔═╡ b2421514-8daa-4214-9e18-736b262b9077
let
	plotly()
